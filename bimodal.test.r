bimodal.test <- function(file_in="test_data.txt", primary_proportion=0.95, var_filter=1.5, method="npEM",file_out="bimodal.test.output.txt"){
...
# with mu0 = 2
# decides if data are unimodal or bimodal + based on proportion
...
dimnames(output)[[2]] <- c("var", "max_portion", "min_portion", "modality_guess", "guess_on")
...
for (i in 1:num_rows){
    ...
    if ( var(my_data[i,])<= var_filter ){
        ...
        if ( identical( method, "npEM" ) ){
            ...
            if  ( my_lambdahat[1] >= primary_proportion ){
                output[i,4] <- "1"
                output[i,5] <- "primary_proportion"
            } else {
...
write.table(output, file=file_out, col.names=NA, row.names=TRUE, append=TRUE, sep="\t")
